...
class GuiGameMaster:
    def __init__(self, game: GameMaster):
        ...
        self.last_move = ''
        self.user_column = ft.Column()
        self.commit_button = ft.Button(content='commit move', on_click=self.commit_move)
    ...
    def load_initial_gui(self):
        ...
        return ft.Row(controls=[self.commit_button, self.market_and_player_board, self.game_bank.gui_obj], spacing=30,
                        vertical_alignment=ft.CrossAxisAlignment.CENTER, alignment=ft.MainAxisAlignment.CENTER)

    def refresh_player_board(self, change_player=False):
        if change_player:
            self.update_current_gui_player()
            for _ in range(3):
                self.market_and_player_board.controls.pop()

        if len(self.market_and_player_board.controls) == 1:  # if its being initialized or changed
    ...
    def update_all_components(self, change_player=False):
        self.market.update_market_grid()
        self.game_bank.update_game_bank_values()
        self.refresh_player_board(change_player=change_player)
    ...
    def commit_move(self):
        ...
        self.update_all_components(change_player=True)
